...
from napari_fluoresfm.fluoresfm.models.unet_attention import SpatialTransformer
...
class UNetModel(nn.Module):
        ...
        *,
        ...
        for i in range(levels):
            ...
            for _ in range(n_res_blocks):
                ...
                layers = [
                    ResBlock(
                        channels, d_time_emb, out_channels=channels_list[i]
                    )
                ]
    ...
    def time_step_embedding(
        self, time_steps: torch.Tensor, max_period: int = 10000
    ):
        """
    ...
    def forward(
        self, x: torch.Tensor, time_steps: torch.Tensor, cond: torch.Tensor
    ):
        """
    ...
    def finetune(self, strategy="in-out"):
        ...
        if strategy == "in-out":
            ...
            raise ValueError(
                f"position should be in-out, in or out, not [{strategy}]."
            )
...
class ResBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor, t_emb: torch.Tensor):
        ...
        if t_emb is not None:
            ...
            h = (
                h + t_emb[:, :, None, None]
            )  # stop using time step embedding (@ Lu)
        # ----------------------------------------------------------------------
