...
class SpatialTransformer(nn.Module):
    ...
    def __init__(
        self, channels: int, n_heads: int, n_layers: int, d_cond: int
    ):
        """
...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_head: int, d_cond: int):
        ...
        if (d_cond is None) or (d_cond == 0):
            self.attn2 = CrossAttention(d_model, d_model, n_heads, d_head)
        else:
...
class CrossAttention(nn.Module):
    ...
    def __init__(
        ...
        try:
            ...
            self.flash = flash_attn_qkvpacked_func
    ...
    def flash_attention(
        ...
        if pad:
            qkv = torch.cat(
                (
                    qkv,
                    qkv.new_zeros(batch_size, seq_len, 3, self.n_heads, pad),
                ),
                dim=-1,
            )
        ...
        out = self.flash(qkv, softmax_scale=self.flash_softmax_scale)
